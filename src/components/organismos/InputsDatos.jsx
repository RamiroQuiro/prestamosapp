...
export default function InputsDatos({label,type,valueInput,name,id,onChange}) {
    return (
        <div className='flex w-[45%] my-1 flex-grow  items-start justify-start px-2'>
            ...
            <input onChange={onChange} name={name} type={type} value={valueInput} className='border-b font border-primary-100/20 active:ring-0  selection:border-transparent  rounded flex-grow selection:ring-0 px-2 valid:ring-0 ' />
        </div>
    )
}

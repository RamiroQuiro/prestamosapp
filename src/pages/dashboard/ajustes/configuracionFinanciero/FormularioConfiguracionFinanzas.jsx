...
export default function FormularioConfiguracionFinanzas({
  dataUser: { intereses, moraCuota, nCuotas },
  usuarioId,
}) {
  ...
  const handleAgregarData = async () => {
    ...
    try {
      const resFetch = await fetch(
        `/api/usuario/configuracionFinanciera/${usuarioId}`,
        {
          method: "POST",
  ...
  const handleChangeRadio = async (e, data) => {
    const selectDefault = e.target.checked;
    ...
    <div className="flex w-full gap-2 flex-col text-sm">
      <div className="flex flex-wrap w-full items-start justify-start gap-y-1 500 flex-col ">
          ...
          <div className="flex items-center text-xs justify-start gap-2">
            ...
            {intereses &&
              intereses.map((interes, i) => {
                ...
                return (
                  <div
                    key={interes.id}
                    className={`${interes.selectDefault&& 'bg-primary-100/20'} border-primary-100/50 mr-2  bg-gray-100 border rounded p-1.5 m-auto relative`}
                  >
                    <input
                      onChange={(e) => handleChangeRadio(e,interes)}
                      type="radio"
                      name="defaultSelect"
                      checked={interes.selectDefault}
                      id=""
            ...
            <label htmlFor="interes" className="">
              <input
                ...
                id="interes"
                className=" border-primary-100 border w-10 rounded p-1.5 m-auto"
              />
            </label>
            ...
            {nCuotas &&
              nCuotas.map((nCuota, i) => {
                ...
                return (
                  <div
                    key={nCuota.id}
                    className="border-primary-100/50 mr-2  text-center bg-gray-100 border w-8 rounded p-1.5 m-auto relative"
                  >
        ...
        <div className="flex items-center justify-start gap-3 my-3">
          <span className="font-semibold">Mora en Cuotas :</span>
          <div className="flex items-center text-xs justify-start gap-2">
            {/* <div className='border-primary-100/50 mr-2 bg-gray-100 border w-7 flex rounded p-1.5 m-auto relative'>
            ...
            {moraCuota &&
              moraCuota.map((mora, i) => (
                <div
                  key={mora.id}
                  className="border-primary-100/50 mr-2 text-center bg-gray-100 border w-12 rounded p-1.5 m-auto relative"
                >
            ...
            <label htmlFor="mora" className="">
              <input
                ...
                id="mora"
                onChange={onChangeForm}
                className=" border-primary-100 border w-8 rounded p-1.5 m-auto"
              />
            </label>

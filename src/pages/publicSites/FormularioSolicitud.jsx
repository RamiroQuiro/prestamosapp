...
export default function FormularioSolicitud({ usuarioId }) {
  ...
  const [formularioSolicitud, setFormularioSolicitud] = useState({nombreApellido:'',dni:0,direccion:'',email:'',celular:0,nCuotas:0,modalidad:'',importe:0});
  const modalidad = [
  ...
  return (
    <div className="w-full">
      {!isPrestamoSimulado ? (
        <form
          action=""
          class="flex flex-col w-full items-center justify-normal p-3 text-primary-texto"
        >
        ...
        <div className="flex flex-col w-full text-g">
          ...
          {prestamosSimulado?.cuotas.slice(0, 3).map((cuota, i) => (
            <div key={i} className="bg-gray-100 px-2 py-1 my-0.5">
              {" "}
              Cuota n°{i + 1}
              {"   -  "}${cuota}
            </div>
          ...
          <button onClick={() => setIsPrestamoSimulado(false)}>
            Volver a simular
          </button>
          <button onClick={() => setIsPrestamoSimulado(false)}>
            Solicitar
